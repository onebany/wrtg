...
### Added
- **Unattended updates, on by default.** Once a day, at 06:00 router time unless changed, `auto-update.sh` runs the same check and install the LuCI Update button does: checksum verified, config kept, service restarted. *Settings → Auto-update* has the switch and the time; the last run's outcome is shown there and on the status page, and goes to syslog as `wrtg: auto-update: …`. A new `setup-cron.sh` owns the two wrtg cron lines and is called by `install.sh` and by the init script on start and reload, so a time saved in LuCI lands with *Save & Reload* and a router upgraded to this build gets the schedule on its first start. Where the ISP blocks the Fastly range GitHub releases live on, the run fails and says so instead of staying silent. `WRTG_AUTO_UPDATE="0"` turns it off.
...
- **Same-account CF Workers each had to earn their own 429** — the Workers Free daily quota is account-wide (100k requests across every Worker on the account, served as HTTP 429), but the cooldown was per-Worker. Each expiring cooldown sent a recovery probe through every configured Worker in turn and collected a guaranteed 429 from every sibling, seen live as all three `*.workers.dev` Workers 429ing within the same second. The first 429 now cools every sibling sharing the account subdomain; Workers on other accounts or custom domains keep independent cooldowns.
- **A Worker without its token was re-dialled forever** — deployed without `WRTG_TOKEN` it fails closed with HTTP 503 on every handshake, and the cooldown ignored anything that was not a 429. Every passthrough and CF fallback paid a doomed WSS handshake and flooded syslog. 503 now earns the same per-Worker backoff. 502 stays exempt: the script serves it for an unreachable destination, which says nothing about the Worker.
- **The installer reported "No CF Worker set" for a router that had three** — the post-install check matched only `CF_WORKER_DOMAIN="..."`, but the config is shell-sourced and the value is commonly written bare. Quoted, single-quoted and bare values all count now, CRLF included.
...
### Fixed
- **Rate-limited CF Workers were re-dialled by every connection** — Cloudflare answers HTTP 429 when a Worker is rate limited, and on the free plan keeps doing so for the rest of the UTC day once the account's 100k-request quota is spent. Nothing backed off: the passthrough path, the MTProto CF-Worker path and the background pool refill each re-dialled *every* configured Worker on *every* connection. Observed on a live router as ~55 failed passthroughs/min against 3 Workers (~165 Worker requests/min ≈ 237k/day, well past the free quota) — the retry storm spent quota it no longer had, added the full connect timeout to every session before falling through to the front, and flooded the syslog ring buffer with 429 warnings (4k lines, cutting log retention to ~18 minutes). A Worker that returns 429 is now skipped until its cooldown expires (60 s, doubling to 15 min; an explicit `Retry-After` wins), and is picked back up as soon as it answers normally. Tunables: `WRTG_CF_WORKER_429_COOLDOWN_SEC` / `WRTG_CF_WORKER_429_MAX_COOLDOWN_SEC`.
- **CF Worker failures were untyped** — `connect_cf_worker_ws` / `connect_cf_worker_tcp` collapsed the typed handshake error into `io::Error`, so callers could not tell 429 from a transport failure without matching on message text (the same trap 0.5.23 removed for redirects). They now return `WsConnectError`, and `wrtg --check` reports a Worker probe failure as a bare `HTTP <code>`.

- **Exponential 429 backoff never escalated** — the cooldown map dropped an entry as soon as it expired, so the next 429 started from zero strikes and the delay stayed pinned at the base value forever (confirmed on a live router: a spent-quota Worker was re-probed every 60 s instead of backing off). An expired entry now keeps its strike count until it has been quiet for a full `max` window, so repeat offenders really do back off 60 s → 120 s → … → 15 min. This also fixes the same latent flaw in the CF-proxy cooldown, which shares the code.

### Changed
- **429 backoff is now shared** — the per-domain cooldown grew a generic `cooldown429::Cooldown429` (mirroring the earlier `ttl_map` / `conn_pool` generalisations); `cf_proxy_cooldown` is a thin adapter over it and `cf_worker_cooldown` is the new Worker-side one. Public API unchanged.
- **Docs** — `config.default` documents the free-plan quota and warns that a large `WRTG_CF_WORKER_POOL_SIZE` burns request quota even while idle, since the pool refills on a timer.
