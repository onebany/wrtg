...
- **Автоматически**: раз в сутки, по умолчанию в 06:00 по часам роутера, wrtg проверяет GitHub и ставит новую версию тем же путём, что кнопка Update: с проверкой контрольной суммы и сохранением конфига. Служба при этом перезапускается. Настройки в **Settings → Автообновление**, в конфиге это `WRTG_AUTO_UPDATE="1"` и `WRTG_AUTO_UPDATE_TIME="06:00"`; результат последнего запуска показан там же и на странице статуса, а также в syslog строкой `wrtg: auto-update: …`. Там, где провайдер режет Fastly, запуск будет падать и говорить об этом, а не молчать.
...
Работает по умолчанию, пока вы не задали свой `CF_PROXY_DOMAIN`. Список из 20 доменов зашит в бинарник и раз в час обновляется из репозитория Flowseal/tg-ws-proxy. На соединение wrtg берёт не больше трёх доменов: первый последовательно, остальные два параллельной гонкой. Домен, который сработал, закрепляется за DC и идёт первым в следующих сессиях; если провалилась вся тройка, курсор DC сдвигается дальше по списку, и следующая сессия пробует другие домены. Часовое обновление списка курсор и закреплённый домен не сбрасывает. На странице статуса это видно в карточке **CF Proxy**, в `--stats` — секция `cf proxy` и счётчик `cf_proxy_dial_failed`.
...
cf worker pool
  ...
  DC3 4
cf proxy
  DC2 sticky kartoshka.co.uk
  DC4 sticky noskomnadzor.co.uk
heap live 745 kB in 744 allocs
  <=64 339 allocs 9 kB
  ...
```
...
| `cf_proxy_dial_failed` сравнимо с `cf_proxy` | Общий пул мигает. Каждая сессия пробует до трёх доменов, провалившаяся тройка сдвигает курсор DC дальше; кто именно отвечает 503 или молчит — `logread -e 'CF proxy'`. |
| `heap live` растёт вместе с RSS | Утечка в демоне, класс размера подскажет чья. Если RSS растёт, а `heap live` стоит, страницы держит аллокатор musl — это не утечка. |

### `wrtg --check`

Резолвит DNS Worker/Proxy-доменов (для общего пула — первые три, через хост `kws2.`, потому что у базовых доменов пула A-записи нет), затем проверяет WSS-handshake для **каждого DC1–DC5** по его реальному пути: front-DC (по умолчанию 2/4) — direct WSS на `FRONT_IP`; остальные — через первый CF Worker (`/apiws?dst=…`), иначе через CF Proxy с тем же бюджетом, что у живой сессии: до трёх доменов пула, первый ответивший засчитывается. Не запускает демон; при ручном запуске конфиг подхватывается из `/etc/wrtg/config` автоматически.
...
Пул неоднороден и непостоянен: один и тот же домен может обслуживать `kws4` и отвечать 404 на `kws2`, а через десять минут поменяться местами с соседом — 503 отдаёт сам Worker, когда у него не отвечает апстрим. Замер на живом пуле дал от 6 до 16 рабочих доменов из 20 в зависимости от DC. Поэтому wrtg ничего не выводит из ротации надолго: провалившаяся тройка просто сдвигает курсор DC дальше по списку. Если пул всё равно часто мимо, поднимите `WRTG_CFPROXY_MAX_ATTEMPTS` — доменов на сессию. Соотношение `cf_proxy_dial_failed` к `cf_proxy` в `--stats` показывает, сколько попыток уходит впустую.
...
| Каждая сессия висит 10 с, потом blind relay | Провайдер закрыл IP Telegram целиком, а пул CF Proxy не ответил. wrtg помнит такой IP (`ip_fail`) и в следующий раз ждёт TCP fallback 2 с вместо 10; убедитесь, что `WRTG_IP_FAIL_COOLDOWN_SEC` не занижен |
...
5. **CF Proxy** — WSS через свой домен либо общий pool (по умолчанию включён, `WRTG_CFPROXY_AUTO="0"` выключает); до 3 доменов, второй и третий идут параллельной гонкой. Сработавший домен закрепляется за DC; провал тройки сдвигает курсор DC дальше по списку.
6. **TCP fallback** — прямое TCP на FRONT_IP или media CDN. Если IP уже помечен недоступным (`ip_fail`), таймаут коннекта 2 с вместо 10.
