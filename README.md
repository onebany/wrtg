...
**Version:** 0.5.8 · **Last updated:** 2026-07-10
...
Опции через env: `VER=v0.5.8`, `WRTG_BASE_URL`, `WRTG_REPO=onebany/wrtg`, `ASSUME_YES=1`, `SKIP_LUCI=1`, `CF_WORKER_DOMAIN=…`.
...
**LuCI** (если не `SKIP_LUCI=1`): **Services → wrtg** — Status, Settings (quick-set + raw config, кнопка `--check`), Logs, Documentation (`/etc/wrtg/README.md`).
...
Проверяет DNS и WSS-handshake для DC2: direct WSS на `FRONT_IP` (оба WS-хоста), CF Worker (`/apiws?dst=…`) и CF Proxy (если настроены). TLS fronting только отображается, если задан `WRTG_FRONTING_SNI` — probe не выполняется. Не запускает демон; на роутере для корректных env лучше через LuCI Settings или `. /etc/wrtg/lib.sh && load_config && wrtg --check`.
