...
const RU = {
	...
	"Once a day wrtg checks GitHub for a newer release and installs it the way the Update button does: checksum verified, config kept, service restarted. Where the ISP blocks GitHub downloads the run fails and is reported here.": "Раз в сутки wrtg проверяет GitHub и ставит новую версию тем же путём, что кнопка Update: с проверкой контрольной суммы и сохранением конфига; служба при этом перезапускается. Там, где провайдер режет загрузки с GitHub, запуск будет падать, и это будет видно здесь.",
	"Update automatically": "Обновлять автоматически",
	"Time (router local)": "Время (по часам роутера)",
	"Last run": "Последний запуск",
	"never": "ещё не было",
	"Not saved: time must be HH:MM: ": "Не сохранено: время должно быть в виде ЧЧ:ММ: ",
	"daily at": "ежедневно в",
	"last run": "последний запуск",
	"own domain": "свой домен",
	"off": "выключен",
	"sessions": "сессий",
	"failed dials": "неудачных попыток",
	"sticky per DC": "закреплённый домен по DC",
	"no sticky yet": "пока нет",
	"The daemon keeps the last domain that worked for each DC and moves past a window that failed.": "Демон запоминает последний рабочий домен для каждого DC и уходит дальше по списку после провалившейся тройки.",
	"CF Worker": "CF Worker",
